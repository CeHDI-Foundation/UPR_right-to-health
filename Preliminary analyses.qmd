...
p <- sdg_data |> 
  ...
  geom_sf(data = state_geo, aes(geometry = polygon, text = country), fill = "grey", alpha = 0.4, lwd=0.01)+
  geom_sf(aes(geometry = polygon, fill = n, text = t_label), lwd=0.01)+
...
p <- sdg_data |> 
  ...
  labs(title = "% of recommendations linked to right to health, per UPR cycle", 
       ...
       caption = "* Cycle 4 is currently underway and the data is incomplete")+
  geom_sf(data = state_geo, aes(geometry = polygon, text = country), fill = "grey", alpha = 0.4, lwd=0.01)+
  geom_sf(aes(geometry = polygon, fill = perc, text = t_label), lwd = 0.01)+
