...
state_geo <- necountries::ne_countries |> 
  ...
  filter(status == "member"|country == "Alaska") |> 
...
upr_data |> 
  ...
  geom_bar(stat = "identity")+
  labs(y = "Number of recommendations", x = "UPR Cycle", 
       title = "UPR recommendations by action category, per cycle",
       fill = NULL)+
  geom_text(aes(label = paste0(round(perc,1), "%")), position = position_stack(vjust = 0.5))+
  geom_text(aes(label = format(n_tot, big.mark = ","), y = n_tot, vjust =-0.2))+
  # geom_text(aes(label = paste0("n = ", format(n, big.mark = ","))))+
...
sdg_data |> 
  group_by(cycle) |> 
  summarise(n = n()/n_distinct(state_under_review))
...
sdg_data |> 
  ...
  ggplot()+
  theme_bw()+
  labs(title = "Number of recommendations issued to states, per cycle", fill = "Number of\nrecommendations")+
  # geom_sf(aes(geometry = polygon, color = wbregion))+
...
sdg_data |> 
  ...
  ggplot()+
  theme_bw()+
  labs(title = "% of recommendations linked to right to health, per cycle", fill = "%")+
  # geom_sf(aes(geometry = polygon, color = wbregion))+
